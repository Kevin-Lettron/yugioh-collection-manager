...
export class UserModel {
  ...
  static async create(username: string, email: string, password: string): Promise<User> {
    ...
    const result = await query(
      `INSERT INTO users (username, email, password_hash)
       VALUES ($1, $2, $3)
       RETURNING id, username, email, profile_picture, role, is_active, created_at, updated_at`,
  ...
  static async findByEmail(email: string): Promise<(User & { password_hash: string }) | null> {
    const result = await query(
      `SELECT id, username, email, password_hash, profile_picture, role, is_active, disabled_at, created_at, updated_at
       FROM users
       WHERE email = $1`,
      [email]
    );
  ...
  static async findById(id: number): Promise<User | null> {
    const result = await query(
      `SELECT id, username, email, profile_picture, role, is_active, created_at, updated_at
       FROM users
       WHERE id = $1`,
      [id]
    );
