...
describe('Mausoleum of White Card Integration', () => {
  ...
  describe('CardModel - Card lookup and storage', () => {
    it('should find Mausoleum of White by card ID', async () => {
      ...
      const card = await CardModel.findByCardId('24382602');
    ...
    it('should find Mausoleum of White by name search', async () => {
      ...
      const card = await CardModel.findByName('Mausoleum of White');
    ...
    it('should upsert Mausoleum of White card data', async () => {
      ...
      const result = await CardModel.upsert({
        card_id: 24382602,
        name: 'Mausoleum of White',
        type: 'Spell Card',
        frame_type: 'spell',
      ...
      expect(mockQuery).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([24382602, 'Mausoleum of White'])
      );
    });
    ...
    it('should search for "Mausoleum" and find the card', async () => {
      ...
      const results = await CardModel.search({ search: 'Mausoleum' });
  ...
  describe('UserCardModel - Collection management', () => {
    ...
    it('should add Mausoleum of White to user collection', async () => {
      ...
      const result = await UserCardModel.addToCollection(userId, cardDbId, {
        quantity: 3,
        set_code: 'SHVI-EN059',
        rarity: 'Common',
      });
    ...
    it('should update quantity of Mausoleum of White in collection', async () => {
      ...
      const result = await UserCardModel.updateQuantity(userId, 1, 2);
    ...
    it('should check if user has Mausoleum of White in collection', async () => {
      ...
      const hasCard = await UserCardModel.hasCard(userId, cardDbId);
    ...
    it('should get total quantity of Mausoleum of White for user', async () => {
      ...
      const total = await UserCardModel.getTotalQuantity(userId, cardDbId);
    ...
    it('should retrieve Mausoleum of White in user collection list', async () => {
      ...
      const collection = await UserCardModel.getUserCollection(userId);
    ...
    it('should remove Mausoleum of White from collection', async () => {
      ...
      const removed = await UserCardModel.removeFromCollection(userId, 1);
  ...
  describe('Card Info Validation', () => {
    ...
    it('should have image URL', () => {
      expect(mausoleumWhiteData.card_images[0].image_url).toBe(
        'https://images.ygoprodeck.com/images/cards/24382602.jpg'
      );
    });
