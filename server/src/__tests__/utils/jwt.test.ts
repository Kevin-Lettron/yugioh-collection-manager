...
describe('JWT Utilities', () => {
  ...
  describe('verifyToken', () => {
    ...
    it('should throw error for expired token', () => {
      ...
      const expiredToken = jwt.sign(mockPayload, process.env.JWT_SECRET || 'test', { expiresIn: '-1s' });

      expect(() => verifyToken(expiredToken)).toThrow();
    });

    it('should throw error for token signed with different secret', () => {
      const tokenWithDifferentSecret = jwt.sign(mockPayload, 'different_secret', { expiresIn: '7d' });
  ...
  describe('generateToken and verifyToken integration', () => {
    ...
    it('should work with special characters in payload', () => {
      ...
      const token = generateToken(payloadWithSpecialChars);
      const decoded = verifyToken(token);
    ...
    it('should work with unicode characters', () => {
      ...
      const token = generateToken(payloadWithUnicode);
      const decoded = verifyToken(token);
