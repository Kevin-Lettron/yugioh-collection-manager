...
class WishlistModel {
     ...
     */
    static async addToWishlist(userId, deckId) {
        try {
            const result = await (0, database_1.query)(`INSERT INTO deck_wishlists (user_id, original_deck_id)
         VALUES ($1, $2)
         ON CONFLICT (user_id, original_deck_id) DO NOTHING
         RETURNING *`, [userId, deckId]);
     ...
     */
    static async removeFromWishlist(userId, deckId) {
        const result = await (0, database_1.query)(`DELETE FROM deck_wishlists WHERE user_id = $1 AND original_deck_id = $2 RETURNING id`, [userId, deckId]);
        return result.rows.length > 0;
    }
    /**
     * Check if deck is in user's wishlist
     */
    static async isInWishlist(userId, deckId) {
        const result = await (0, database_1.query)(`SELECT id FROM deck_wishlists WHERE user_id = $1 AND original_deck_id = $2`, [userId, deckId]);
        return result.rows.length > 0;
    }
    /**
     * Get user's wishlist
    ...
    static async getUserWishlist(userId, limit = 50, offset = 0) {
        // Get total count
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) as count FROM deck_wishlists WHERE user_id = $1`, [userId]);
        const total = parseInt(countResult.rows[0].count);
        // Get wishlist with deck details
        const result = await (0, database_1.query)(`SELECT dw.*,
     ...
     */
    static async getWishlistCount(deckId) {
        const result = await (0, database_1.query)(`SELECT COUNT(*) as count FROM deck_wishlists WHERE original_deck_id = $1`, [deckId]);
