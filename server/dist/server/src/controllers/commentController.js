...
class CommentController {
    ...
    static async createComment(req, res, next) {
        try {
            ...
            if (req.params.deckId) {
                ...
                const parentComment = await deckCommentModel_1.DeckCommentModel.findById(parentId);
                if (!parentComment) {
                ...
                throw new errorHandler_1.ValidationError('Invalid request');
            }
            if (isNaN(deckId)) {
            ...
            const comment = await deckCommentModel_1.DeckCommentModel.create(req.user.id, deckId, content.trim(), parentCommentIdToUse);
            // Create notification
            if (parentCommentIdToUse) {
                // Notification for reply to comment
                const parentComment = await deckCommentModel_1.DeckCommentModel.findById(parentCommentIdToUse);
    ...
    static async getComments(req, res, next) {
        try {
            ...
            const comments = await deckCommentModel_1.DeckCommentModel.getDeckCommentsWithReplies(deckId);
            res.json({
                comments: comments,
