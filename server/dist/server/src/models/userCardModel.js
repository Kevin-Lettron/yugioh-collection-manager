...
class UserCardModel {
     ...
     */
    static async addToCollection(userId, cardId, setCode, rarity, quantity = 1, language = 'EN') {
        const result = await (0, database_1.query)(`INSERT INTO user_cards (user_id, card_id, set_code, rarity, quantity, language)
       VALUES ($1, $2, $3, $4, $5, $6)
       ...
       RETURNING *`, [userId, cardId, setCode, rarity, quantity, language]);
    ...
    static async getUserCard(userId, cardId) {
       ...
       FROM user_cards uc
       JOIN cards c ON uc.card_id = c.id
       WHERE uc.user_id = $1 AND uc.card_id = $2`, [userId, cardId]);
        return result.rows[0] ? this.parseUserCard(result.rows[0]) : null;
    }
    /**
     * Check if user has card in collection
     */
    static async hasCard(userId, cardId) {
        const result = await (0, database_1.query)(`SELECT COUNT(*) as count FROM user_cards WHERE user_id = $1 AND card_id = $2`, [userId, cardId]);
        return parseInt(result.rows[0].count) > 0;
    }
    /**
     * Get total quantity of a card in user's collection (all rarities)
     */
    static async getTotalQuantity(userId, cardId) {
        const result = await (0, database_1.query)(`SELECT SUM(quantity) as total FROM user_cards WHERE user_id = $1 AND card_id = $2`, [userId, cardId]);
        return parseInt(result.rows[0].total || 0);
    }
    /**
     * Update card quantity
     */
    static async updateQuantity(userId, userCardId, quantity) {
        if (quantity <= 0) {
            await (0, database_1.query)(`DELETE FROM user_cards WHERE id = $1 AND user_id = $2`, [userCardId, userId]);
            return null;
        }
        const result = await (0, database_1.query)(`UPDATE user_cards
       SET quantity = $1, updated_at = CURRENT_TIMESTAMP
       WHERE id = $2 AND user_id = $3
       RETURNING *`, [quantity, userCardId, userId]);
        return result.rows[0] || null;
    }
    /**
     * Remove card from collection
     */
    static async removeFromCollection(userId, userCardId) {
        const result = await (0, database_1.query)(`DELETE FROM user_cards WHERE id = $1 AND user_id = $2 RETURNING id`, [userCardId, userId]);
