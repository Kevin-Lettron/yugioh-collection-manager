...
class DeckModel {
     ...
     * Create a new deck
     */
    static async create(userId, name, respectBanlist = true, isPublic = true, coverImage) {
        const result = await (0, database_1.query)(`INSERT INTO decks (user_id, name, respect_banlist, is_public, cover_image)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [userId, name, respectBanlist, isPublic, coverImage || null]);
    ...
    static async findById(deckId, requestingUserId) {
       ...
       ORDER BY c.name`, [deckId]);
        ...
        if (requestingUserId) {
            const reactionResult = await (0, database_1.query)(`SELECT dr.is_like FROM deck_reactions dr WHERE dr.deck_id = $1 AND dr.user_id = $2`, [deckId, requestingUserId]);
            if (reactionResult.rows.length > 0) {
                userReaction = reactionResult.rows[0].is_like ? 'like' : 'dislike';
            }
            // Check if wishlisted
            const wishlistResult = await (0, database_1.query)(`SELECT dw.id FROM deck_wishlists dw WHERE dw.original_deck_id = $1 AND dw.user_id = $2`, [deckId, requestingUserId]);
     ...
     */
    static async getUserDecks(userId, filters = {}) {
        ...
        // Get total count
        const countResult = await (0, database_1.query)(`SELECT COUNT(*) as count FROM decks d WHERE ${whereClause}`, values);
       ...
       LIMIT $${paramCount} OFFSET $${paramCount + 1}`, values);
     ...
     * Search public decks
     */
    static async searchPublicDecks(filters = {}, requestingUserId) {
     ...
     * Update deck
     */
    static async update(deckId, userId, updates) {
        const fields = [];
        const values = [];
        let paramCount = 1;
    ...
    static async delete(deckId, userId) {
        const result = await (0, database_1.query)(`DELETE FROM decks WHERE id = $1 AND user_id = $2 RETURNING id`, [deckId, userId]);
     ...
     */
    static async addCard(deckId, userId, cardId, quantity, isExtraDeck, card) {
        // Verify deck ownership
        ...
        const cardIsExtraDeck = ygoprodeckService_1.YGOProDeckService.isExtraDeckCard(card.frame_type);
        ...
        const countResult = await (0, database_1.query)(`SELECT
       ...
       WHERE deck_id = $1`, [deckId, cardId]);
        ...
        if (deck.respect_banlist) {
            const banlistLimit = ygoprodeckService_1.YGOProDeckService.getBanlistLimit(card);
            if (banlistLimit === 0) {
                return {
                    success: false,
        ...
        const existingCard = await (0, database_1.query)(`SELECT id FROM deck_cards WHERE deck_id = $1 AND card_id = $2 AND is_extra_deck = $3`, [deckId, cardId, isExtraDeck]);
        if (existingCard.rows.length > 0) {
            // Update existing card
            await (0, database_1.query)(`UPDATE deck_cards SET quantity = $1 WHERE deck_id = $2 AND card_id = $3 AND is_extra_deck = $4`, [quantity, deckId, cardId, isExtraDeck]);
        }
        else {
            // Insert new card
            await (0, database_1.query)(`INSERT INTO deck_cards (deck_id, card_id, quantity, is_extra_deck)
         VALUES ($1, $2, $3, $4)`, [deckId, cardId, quantity, isExtraDeck]);
        }
        // Update deck timestamp
        await (0, database_1.query)(`UPDATE decks SET updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [deckId]);
        return { success: true };
    }
    /**
     * Clear all cards from deck
     */
    static async clearCards(deckId, userId) {
        // Verify deck ownership
        const deckResult = await (0, database_1.query)(`SELECT id FROM decks WHERE id = $1 AND user_id = $2`, [deckId, userId]);
        if (deckResult.rows.length === 0) {
            return false;
        }
        await (0, database_1.query)(`DELETE FROM deck_cards WHERE deck_id = $1`, [deckId]);
        await (0, database_1.query)(`UPDATE decks SET updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [deckId]);
        return true;
    }
    /**
    ...
    static async removeCard(deckId, userId, deckCardId) {
        ...
        const result = await (0, database_1.query)(`DELETE FROM deck_cards WHERE id = $1 AND deck_id = $2 RETURNING id`, [deckCardId, deckId]);
        if (result.rows.length > 0) {
            await (0, database_1.query)(`UPDATE decks SET updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [deckId]);
            return true;
        }
     ...
     */
    static async updateCardQuantity(deckId, userId, deckCardId, quantity, card) {
        // Verify deck ownership
        const deck = await this.findById(deckId, userId);
        if (!deck || deck.user_id !== userId) {
            return { success: false, error: 'Deck not found or unauthorized' };
        ...
        const cardResult = await (0, database_1.query)(`SELECT card_id, quantity, is_extra_deck FROM deck_cards WHERE id = $1 AND deck_id = $2`, [deckCardId, deckId]);
        ...
        const countResult = await (0, database_1.query)(`SELECT COALESCE(SUM(quantity), 0) as total_copies
       FROM deck_cards
       WHERE deck_id = $1 AND card_id = $2`, [deckId, card_id]);
        ...
        // Update quantity
        await (0, database_1.query)(`UPDATE deck_cards SET quantity = $1 WHERE id = $2 AND deck_id = $3`, [quantity, deckCardId, deckId]);
        await (0, database_1.query)(`UPDATE decks SET updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [deckId]);
        return { success: true };
    }
    /**
     ...
     */
    static async validateDeck(deckId) {
        const countResult = await (0, database_1.query)(`SELECT
        COALESCE(SUM(CASE WHEN is_extra_deck = false THEN quantity ELSE 0 END), 0) as main_count,
        COALESCE(SUM(CASE WHEN is_extra_deck = true THEN quantity ELSE 0 END), 0) as extra_count
       FROM deck_cards
